...
func (s *taskDALImp) GetForUpdate(tx *gorm.DB, id uint64) (*Task, error) {
	var rule Task
	if err := tx.Table(s.config.TableName).Where("id = ?", id).Clauses(clause.Locking{Strength: "UPDATE"}).Take(
		&rule).Error; err == gorm.ErrRecordNotFound {
		return nil, nil
	} else if err != nil {
		return nil, err
	}
...
func (s *taskDALImp) Update(tx *gorm.DB, task *Task) (int64, error) {
	db := tx.Table(s.config.TableName).Updates(task)
	return db.RowsAffected, db.Error
}

func (s *taskDALImp) UpdateStatus(tx *gorm.DB, task Task, newStatus TaskStatus) (int64, error) {
	db := tx.Table(s.config.TableName).Where("id = ? AND task_status = ?", task.ID, task.TaskStatus).
		Updates(&Task{TaskStatus: newStatus})
	return db.RowsAffected, db.Error
}

func (s *taskDALImp) UpdateStatusByIDs(tx *gorm.DB, ids []uint64, oriStatus TaskStatus, newStatus TaskStatus) (int64, error) {
	db := tx.Table(s.config.TableName).Where("id IN (?) AND task_status = ?", ids, oriStatus).
		Updates(&Task{TaskStatus: newStatus})
	return db.RowsAffected, db.Error
}
